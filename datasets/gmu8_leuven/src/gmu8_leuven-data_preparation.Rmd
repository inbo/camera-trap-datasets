---
title: "Prepare gmu8_leuven dataset for publication to GBIF"
author: 
- Sanne Govaert
- Jim Casaer
date: "`r Sys.Date()`"
...
```{r camtrap data}
name_exportfolder <- "gmu8leuven-20250904102618"
...
agouti_id <- "91e1e104-a296-461e-af79-eeb6cb3e6c7f"
```
...
Remove exotic species that are also game species:

```{r}
game_species <- c("Branta canadensis", "Dama dama", "Phasianus colchicus")
exotic_species <- exotic_species[!exotic_species %in% game_species]
```

Define embargo date:

```{r}
embargo_date <- Sys.Date() %m-% months(12) # Last one used: "2024-09-05"
```
...
Camtrap DP does not accept negative values in bounding box values, so set those to `NA`:
