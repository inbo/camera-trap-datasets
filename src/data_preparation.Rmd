...
```{r camtrap data}
name_exportfolder <- "muntjak-vlaanderen-20241107102959"
file <- here::here("data", name_exportfolder, "raw", "datapackage.json")
...
muntjac$description <- c(
  paste0("<i>", muntjac$title, "</i> is a camera trap dataset published by the <a href=\"https://www.inbo.be/en\">Research Institute of Nature and Forest (INBO)</a>. It contains animal tracking data collected for the monitoring of Chinese muntjac (<i>Muntiacus reevesi</i>) in the Region of Flanders with respect to Regulation EU nr. 1143/2014. The monitoring is performed in close collaboration with the Agency for Nature and Forests (ANB). Issues with the dataset can be reported at <a href=\"https://github.com/inbo/camera-trap-datasets/issues\">https://github.com/inbo/camera-trap-datasets/issues</a>."),
...
```{r write dwc}
camtrapdp::write_dwc(muntjac, here::here("data", name_exportfolder, "processed", "dwc"))
```
...
camtrapdp::write_camtrapdp(muntjac, here::here("data", name_exportfolder, "processed", "package"))
```

# Write and transform EML

```{r write EML}
eml <- camtrapdp::write_eml(muntjac, here::here("data", name_exportfolder, "processed", "dwc"), derived_paragraph = TRUE)
```
...
eml$dataset$metadataProvider <- EML::set_responsibleParty(
  ...
  userId = list(directory = "https://orcid.org/", "0000-0002-8939-1305")
)
...
taxonomic_coverage <- data.frame(
  scientificName = c("Aves", "Mammalia"),
  taxonRank = c("class", "class")
)
...
# Sampling methods
study_extent <- "Several forest areas east of Antwerp were sampled for animal wildlife; since Chinese muntjac is known to occur in the wider region since about 2012. The main area of study is Park Vordenstein; a government-owned woodland park that is open to the public."

sampling_description <- "Camera traps were installed per forest area; in northerly direction at about 50 cm of height. The number of camera traps varied per area and per year. Images from the camera traps were loaded into Agouti for data annotation."

quality_control <- "All observations of muntjac were double-checked in the framework of further analyses."

steps <- c(
  "Allocation of camera trap locations.",
  "Deployment of cameras.",
  "Reading out of images.",
  "Annotation of images.",
  "Analysis and data publication."
)

eml$dataset$methods <- purrr::map(
  steps, ~ list(description = list(para = .x))
  ) %>% 
  purrr::set_names("methodStep")

eml$dataset$methods$sampling$studyExtent$description$para <- study_extent
eml$dataset$methods$sampling$samplingDescription$para <- sampling_description
eml$dataset$methods$qualityControl$description$para <- quality_control

# Bibliographic Citations
eml$additionalMetadata$metadata$gbif$bibliography$bibliography$citation <- "D’hondt; B.; Engels; P.; Verbist; B.; Vercammen; J.; Huygens; W.; Adriaens; T.; and Casaer; J. (2023). Monitoring van een Vlaamse populatie van Chinese muntjak (Muntiacus reevesi) in het kader van bestrijding. Lutra 66: 105-121. <a href=\"https://pureportal.inbo.be/files/101748474/Lutra_66_2_D_hondt_et_al_2023.pdf\">https://pureportal.inbo.be/files/101748474/Lutra_66_2_D_hondt_et_al_2023.pdf</a>"

# Remove project data
eml$dataset$project <- NULL
```

Write EML again.

```{r overwrite EML}
EML::write_eml(eml, here::here("data", name_exportfolder, "processed", "dwc", "eml.xml"))
```
