...
```{r camtrap data}
name_exportfolder <- "muntjak-vlaanderen-20241105085239"
...
```{r write dwc}
camtrapdp::write_dwc(muntjac, here::here("data", name_exportfolder, "processed"))
```
...
camtrapdp::write_camtrapdp(muntjac, here::here("data", name_exportfolder, "processed"))
```

# Write and transform EML
```{r write EML}
eml <- camtrapdp::write_eml(muntjac, here::here("data", name_exportfolder, "processed"), derived_paragraph = TRUE)
```
...
```{r overwrite EML}
EML::write_eml(eml, here::here("data", name_exportfolder, "processed", "eml.xml"))
```
