...
observations_clean %>% filter(deploymentID == "3ca9d6dd-2dd3-4f68-adf1-316466ea16cf")
```
...
# check
deployments_clean %>% filter(deploymentID == deploymentID_storm) %>% select(deploymentEnd)

# add updated deployments and observations to data package
data$deployments <- deployments_clean 
data$observations <- observations_clean 
...
data <-
  data %>% 
  camtrapdp::filter_deployments(deploymentID != "91ae688c-818c-4e74-8c02-8bc2f8c842be")
```
...
data <-
  data %>% 
  camtrapdp::filter_deployments(deploymentID != "72633b07-b39c-4827-ae35-ffcc1cb02607")
```
...
filtered_data %>% 
  filter_observations(scientificName == "Canis lupus familiaris") %>% 
  media()
```
