...
```{r camtrap data}
name_exportfolder <- "muntjak-vlaanderen-20241010093834"
...
Dogs (*Canis lupus*) are incorrectly identified as wolves (*Canis lupus familiaris*). This mistake is fixed.

```{r correct wolves to dogs}
camtrapdp::observations(clean_data) <- 
  camtrapdp::observations(clean_data) %>% 
...
```{r write camtrap DP}
camtrapdp:::version(muntjac) <- "1.0.0"
