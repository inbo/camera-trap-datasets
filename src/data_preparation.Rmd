...
# check
deployments %>% filter(deploymentID == deploymentID_storm) %>% select(deploymentEnd)

# add updated deployments to data package
...
# Check if fix is correct
observations(data) %>% 
  filter(deploymentID == deploymentID_storm) %>% 
  summarize(
    last_eventEnd = max(eventEnd)
  )

# work with new data frames
media <- camtrapdp::media(data)
observations <- camtrapdp::observations(data)
```
...
data <-
  data %>% 
  camtrapdp::filter_deployments(deploymentID != "7a85b0ea-6ed8-4d1e-801f-78082cb660ec")
```
...
Exclude
records that are blank (observationType)
unclassified media
vehicles
